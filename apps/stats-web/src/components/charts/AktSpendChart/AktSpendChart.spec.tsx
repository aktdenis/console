...
import { AktSpendChart, type AktSpendChartProps, DEPENDENCIES } from "@/components/charts/AktSpendChart/AktSpendChart";
...
describe(AktSpendChart.name, () => {
  ...
  it("converts micro-denominated values before charting", () => {
    ...
    expect(deps.AreaChart.mock.calls.at(-1)?.at(0)?.data).toEqual([{ date: "2026-07-01", dailyAktSpent: 11618.11666 }]);
  });
  ...
  it("labels the latest value as AKT, not a bare number", () => {
  ...
  function setup(props: AktSpendChartProps) {
    const deps = MockComponents(DEPENDENCIES);
    const result = render(<AktSpendChart {...props} dependencies={deps} />, {
